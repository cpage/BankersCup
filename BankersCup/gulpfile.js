...
var config = {
    ...
    boostrapfonts: 'bower_components/bootstrap/dist/fonts/**/*.{eot,ttf,woff,eof,svg}',
...
gulp.task('css', ['clean-styles', 'bower-restore'], function () {
    return gulp.src([config.bootstrapcss, config.appcss, config.appfontcss])
     .pipe(concat('app.css'))
...
gulp.task('fonts', ['clean-styles', 'bower-restore'], function () {
    gulp.src([config.boostrapfonts, config.appfonts])
